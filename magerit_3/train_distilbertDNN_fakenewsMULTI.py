...
import torch
import torch.nn as nn  # NEW: Imported for DataParallel
...
configs = [
    ...
    {"name": "2", "epochs": 1, "per_device_train_batch_size": BATCH_SIZE, "learning_rate": 1.5e-5,
     "weight_decay": 0.02},
    {"name": "3", "epochs": 2, "per_device_train_batch_size": BATCH_SIZE, "learning_rate": 3e-5, "weight_decay": 0.02},
    {"name": "4", "epochs": 2, "per_device_train_batch_size": BATCH_SIZE, "learning_rate": 1.5e-5,
     "weight_decay": 0.02},
    {"name": "5", "epochs": 3, "per_device_train_batch_size": BATCH_SIZE, "learning_rate": 3e-5, "weight_decay": 0.02},
    {"name": "6", "epochs": 3, "per_device_train_batch_size": BATCH_SIZE, "learning_rate": 1.5e-5,
     "weight_decay": 0.02},
]
...
def train_epoch(model, data_loader, optimizer, scheduler, device):
    ...
    for batch in progress_bar:
        ...
        # NEW: Handle loss extraction when using DataParallel
        # DataParallel returns a vector of losses (one for each GPU). We need to mean() it.
        loss = outputs.loss
...
def main():
    # NEW: Check how many GPUs are available
    n_gpu = torch.cuda.device_count()
    if n_gpu > 1:
        print(f"Detected {n_gpu} GPUs! Parallelizing training via DataParallel.")
    ...
    for cfg in configs:
        ...
        print("=" * 60)
        print(
            f"Starting run for CONFIG {cfg['name']} | epochs={cfg['epochs']}, lr={cfg['learning_rate']}, wd={cfg['weight_decay']}, batch_size={cfg['per_device_train_batch_size']}, device={device}, max_length={MAX_LENGTH}")
        print("=" * 60)
        ...
        for run in range(1, 7):
            ...
            # NEW: Wrap the model in DataParallel if more than 1 GPU exists
            if n_gpu > 1:
            ...
            for epoch in range(cfg['epochs']):
                ...
                if val_acc > best_val_acc:
                    ...
                    # NEW: DataParallel modifies the state_dict keys (adds 'module.').
                    # We must save the underlying original model weights so it can be loaded normally later.
                    if isinstance(model, nn.DataParallel):
